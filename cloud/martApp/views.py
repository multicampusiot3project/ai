...
def index(request):
    ...
    routes = list(range(0, 100, 1))                     # 초기 세팅 우측 테이블 갯수 미리 만들어 놓음
    context = {'rows': rows,
               'cols': cols,
               'routes': routes,
               }
...
def def_route(now_position):
    ...
    route_list = ["D_4", "E_10", "I_1"]    # 거처여할곳(상품 있는곳 옆) 나중에 DB에서 긁어와야함
...
def find_junction(first, second):
    ...
    # print("junction : ", type(junction))
    # print("junction : ", len(junction))
    junction_list = []
    for i in junction:
